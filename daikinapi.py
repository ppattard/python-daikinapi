...
class Daikin:
    ...
    def hourly_consumption(self):
        """
        power consumption in 0.1kWh for past 48 hours
        (first point is yesterday 00:00, last point is today 23:00)
        :return: a dict with breadown heat/cool
        """
        day_power = self._get_day_power()
        return {
            'heat': [int(c) * 1.0 / 10 for c in day_power["prev_1day_heat"].split("/")]
                + [int(c) * 1.0 / 10 for c in day_power["curr_day_heat"].split("/")],
            'cool': [int(c) * 1.0 / 10 for c in day_power["prev_1day_cool"].split("/")]
                + [int(c) * 1.0 / 10 for c in day_power["curr_day_cool"].split("/")]
        }
